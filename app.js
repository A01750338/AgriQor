...
app.post('/register', async (req, res) => {
  try {
    ...
    const {nombre, numero, contrasena,fecha,genero,relacion,estado} = req.body;

    const sqlInsert = 'INSERT INTO usuario (Nombre,Genero,Nacimiento,Edad,Ubicacion,Relacion,Numero,Contrasena) VALUES (?,?,?,?,?)';
    const result = await db.query(sqlInsert, [nombre,genero,fecha,estado,'NULL',relacion,numero,contrasena]);

    res.type('text').status.send(`Usuario creado con el ID = ${ result.insertId }.\n`);
    return;
